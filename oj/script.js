...
const HIDE_TAGS_KEY = 'oj_hide_tags';
...
function getCurrentUsername() {
    if (typeof get_cur_user === 'function') {
        return get_cur_user();
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadState();
    loadFontScale();
    loadHideTagsPreference();
    setupEventListeners();
    populateFilters();
    restoreFilterUI();
    renderQuestions();
    updateStats();          // 初始加载时也会保存分数（可能为0）
    setupClock();
    setupLogout();
    applyFontScale();
...
function setupEventListeners() {
    ...
    if (toggleTagsBtn) toggleTagsBtn.addEventListener('click', toggleTagVisibility);
}
...
function applyTagVisibility() {
    ...
    if (container) {
        if (hideTags) container.classList.add('hide-question-tags');
        else container.classList.remove('hide-question-tags');
    }
}
...
function updateTagToggleButton() {
    ...
    if (btn) {
        btn.innerHTML = hideTags ? '🏷️ Show Tags' : '🏷️ Hide Tags';
        btn.title = hideTags ? 'Show question metadata tags' : 'Hide question metadata tags';
    }
}
...
function applyFontScale() {
    const container = document.getElementById('questionsContainer');
    if (container) container.style.setProperty('--problem-scale', currentFontScale);
}

function updateFontSizePercentDisplay() {
    const percentSpan = document.getElementById('fontSizePercent');
    if (percentSpan) percentSpan.textContent = `${Math.round(currentFontScale * 100)}%`;
}
...
function loadFontScale() {
    ...
    if (saved !== null) {
        ...
        if (!isNaN(parsed) && parsed >= MIN_SCALE && parsed <= MAX_SCALE) currentFontScale = parsed;
        else currentFontScale = 1.0;
    } else currentFontScale = 1.0;
}
...
function populateTopicFilter() {
    ...
    if (selectedSubject !== 'all') {
        const topics = [...new Set(database.filter(q => q.subject === selectedSubject).map(q => q.topic))].sort();
...
function createQuestionCard(q, qIndex) {
    ...
    let answerHtml = '';
    if (q.type === 'mc') answerHtml = createMCAnswerArea(q, qIndex);
    else answerHtml = createLQAnswerArea(q, qIndex);
...
function createMCAnswerArea(q, qIndex) {
    ...
    const optionsHtml = q.choice.map((choice, idx) => {
        ...
        return `
            <label class="option">
                <input type="radio" name="q_${qIndex}" value="${letter}" ${selectedAnswer === letter ? 'checked' : ''} ${isSubmitted ? 'disabled' : ''}>
    ...
    const actionButtonsHtml = `
        <div class="question-actions">
            <button class="btn-submit-q" onclick="submitMC(${qIndex})" ${isSubmitted ? 'disabled' : ''}>${isSubmitted ? 'Submitted' : 'Submit'}</button>
            <button class="btn-clear-q" onclick="clearQuestion(${qIndex})">Clear</button>
        </div>
    `;
    return `<div class="options">${optionsHtml}</div>${answerSectionHtml}${actionButtonsHtml}`;
}
...
function createLQAnswerArea(q, qIndex) {
    ...
    let html = `<textarea class="answer-textarea" id="textarea_${qIndex}" ${isSubmitted ? 'disabled' : ''} placeholder="Enter your answer here...">${userAnswer}</textarea>`;
    if (isSubmitted) {
        html += `
            <div class="answer-section show">
                ...
                <div class="points-input-area">
                    ...
                    <input type="number" id="points_${qIndex}" min="0" max="${q.point}" value="${earnedPoints || 0}" placeholder="0">
    ...
    const actionButtonsHtml = `
        <div class="question-actions">
            <button class="btn-submit-q" onclick="submitLQ(${qIndex})" ${isSubmitted && earnedPoints !== undefined ? 'disabled' : ''}>${isSubmitted ? 'Submitted' : 'Submit'}</button>
            <button class="btn-clear-q" onclick="clearQuestion(${qIndex})">Clear</button>
        </div>
    `;
...
function createParticleEffect(button, color) {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        let x = startX, y = startY, opacity = 1;
        const duration = 600;
        ...
        const animate = () => {
            ...
            if (progress < 1) {
                x += vx; y += vy; vy += 0.2; opacity = 1 - progress;
...
function saveLQPoints(qIndex, maxPoints) {
    ...
    if (points === maxPoints) particleColor = '#28a745';
    else if (points === 0) particleColor = '#DC3545';
    ...
    filteredQuestions.forEach((q, idx) => {
        ...
        if (!state.submitted[qIndex]) {
            if (q.type === 'mc') {
                ...
                for (let radio of radios) { if (radio.checked) { isAnswered = true; break; } }
                if (isAnswered) submitMC(qIndex);
            } else {
                const textarea = document.getElementById(`textarea_${qIndex}`);
                if (textarea && textarea.value.trim()) submitLQ(qIndex);
            }
        }
    });
...
function updateStats() {
    ...
    filteredQuestions.forEach(q => {
        ...
        if (state.submitted[qIndex]) {
            submittedCount++;
            if (q.type === 'mc') {
                if (state.answers[qIndex] === q.letter) earnedPoints += q.point;
            } else {
    ...
    // 保存当前总分到 localStorage（关联当前用户）
    saveTotalScoreToLocalStorage(earnedPoints);
}

function saveTotalScoreToLocalStorage(score) {
    const username = getCurrentUsername();
    if (username && username !== 'null' && username !== 'undefined') {
        const scoreKey = `oj_score_${username}`;
        localStorage.setItem(scoreKey, score.toString());
        console.log(`Score saved: ${score} for user ${username}`);
    } else {
        console.warn('Cannot save score: no valid username');
    }
}
...
function loadState() {
    ...
    if (stored) {
        try { state = JSON.parse(stored); } catch(e) { console.error(e); }
    }
    const storedFilters = localStorage.getItem(FILTERS_STORAGE_KEY);
    if (storedFilters) {
        try { state.currentFilters = JSON.parse(storedFilters); } catch(e) { console.error(e); }
    }
}
